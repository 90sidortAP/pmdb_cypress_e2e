...
context('PMDB: Template Basic tests', () => {
    ...
    it('Should be able to open and close fullscreen template view', () => {
        ...
        cy.get(iconFullClose, { timeout: timeWait })
    ...
    it('Should be able to open template edit mode in full view', () => {
        cy.openTemplateDetails(99)
        cy.get(fullViewIcon).click()
        cy.get(buttonEditFull).eq(1).click()
    ...
    it('Should be possible to make changes to an existing template', () => {
        ...
        cy.get(templateEditDetail).eq(0).click()
        cy.get(buttonSaveEdit).contains('Save').should('exist')
        cy.get(editInputApi).clear()
        cy.get(editInputApi).type(templateValueId)
        cy.get(buttonSaveEdit).contains('Save').click()
        cy.get(editNewCreated).should('have.text',templateSuccessUpd)
    ...
    it('Should be impossible to create new template without mandatory data', () => {
        ...
        cy.get(templateSelSource).eq(0)
            ...
            .select("License-in")
        cy.get(templateCreate).click()
        cy.get(editPM).should('exist').click()   
        cy.get(editInput).type(managerPiwo)
        cy.get('li').contains(managerPiwo).click()
        cy.wait(500)
        cy.get(templateCreate).click()
        cy.get(templateSelMol).should('exist').click()
        cy.get(editInput).type(template1Prop)
    ...
    it('Should be possible to save changest made to template Schedule section', () => {
        ...
        cy.get(templateEditDetail).eq(0).click()
        cy.get(buttonSaveEdit).contains('Save').should('exist')
        cy.get(editPMBApp, { timeout : timeWait }).clear().type('2020-05-30')
        cy.wait(500)
        cy.window().then((win) => {
            win.document.querySelector(richTextEditor).textContent=templateChangeTest
        })
        cy.wait(500)
        cy.get(templateIPCX).contains(templateChangeTest)
        cy.get(buttonSaveEdit).eq(1).click({ force: true })
        cy.get(editNewCreated, { timeout: timeWait }).should('have.text',templateSuccessUpd)
    ...
    it('Should be possible to save changest made to template Finance section', () => {
        ...
        cy.get(templateEditDetail).eq(0).click()
        cy.get(buttonSaveEdit).contains('Save').should('exist')
        cy.get(editPBudget, {timeout: timeWait }).clear().type(templateValueId)
    ...
    it('Should be possible to save changest made to template Others section', () => {
        ...
        cy.window().then((win) => {
            win.document.querySelectorAll(richTextEditor)[5].textContent = templateChangeTest
        })
    ...
    it('Should be possible to create new template with mandatory data', () => {
        ...
        cy.get(editInput).type(formHerbal)
        cy.get('li').contains(formHerbal).click()
        cy.get(templateCreate).click()
        cy.get(editNewCreated).should('have.text', templateSuccessNfo)
        cy.wait(1000)
    ...
    it('Should be possible to create new template with all data', () => {
        ...
        cy.get(editInput).type(templateNeurology)
        cy.get('li').contains(templateNeurology).click()
        cy.get(templateSelATC).click()
        cy.get(editInput).type(templateAntimeta)
